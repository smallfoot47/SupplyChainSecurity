...
def get_log_entry(log_index, debug=False):
    ...
    base_url = f"https://rekor.sigstore.dev/api/v1/log/entries"
    params = { "logIndex" : log_index }

    try:
        data = requests.get(base_url, params=params).json()[0]
        return data
...
def get_verification_proof(log_index, debug=False):
    base_url = f"https://rekor.sigstore.dev/api/v1/log"
    params = { "logIndex" : log_index }

    try:
        data = requests.get(base_url, params=params).json()[0]["verification"]["inclusionProof"]
        return data
    except requests.RequestException as e:
        if debug:
            print(f"Error: {e}")
